...
inline diva::Diva<diva::DivaType::BinaryTrie> *init(const t_itr begin, const t_itr end, const double bpk) {
    const uint32_t rng_seed = 1024;
    const double load_factor = 0.95;
    const uint32_t infix_size = std::round(load_factor * (bpk - 1));
    
    auto *filter = new diva::Diva<diva::DivaType::BinaryTrie>(infix_size,
                                                            begin, end,
                                                            rng_seed,
                                                            load_factor);
    return filter;
...
inline void insert(diva::Diva<diva::DivaType::BinaryTrie> *filter,
                   const uint8_t *key, uint16_t key_length) {
    filter->Insert(key, key_length);
}

inline void insert_concurrent(diva::Diva<diva::DivaType::BinaryTrie> *filter,
                              const uint8_t * key, uint16_t key_length,
                              uint32_t thread_id) {
...
inline size_t size(const diva::Diva<diva::DivaType::BinaryTrie> *filter) {
    return filter->Size();
}
