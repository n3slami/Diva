...
TEST_SUITE("node 256") {
  ...
  TEST_CASE("next partial key") {
    ...
    SUBCASE("child at 0") {
      ...
      for (int i = 1; i < 256; ++i) {
        REQUIRE_THROWS_AS(n.next_partial_key(i), std::out_of_range);
      }
    }
    ...
    SUBCASE("child at 255") {
      ...
      n.set_child(255, &n0);
      for (int i = 0; i < 256; ++i) {
        REQUIRE_EQ(255, n.next_partial_key(i));
      }
    }
  ...
  TEST_CASE("previous partial key") {
    ...
    SUBCASE("completely empty node") {
      for (int i = 0; i < 256; ++i) {
        REQUIRE_THROWS_AS(n.prev_partial_key(i), std::out_of_range);
      }
    }
    ...
    SUBCASE("child at 0") {
      ...
      n.set_child(0, &n0);
      for (int i = 0; i < 256; ++i) {
        REQUIRE_EQ(0, n.prev_partial_key(i));
      }
    }
