...
public:
    ...
    static void Insert() {
        ...
        Steroids<O> s(infix_size, seed, load_factor);
        ...
        SUBCASE("interpolated inserts") {
            ...
            typename Steroids<O>::InfixStore *store;
        ...
        SUBCASE("split infix store: right half") {
            ...
            if constexpr (O) {
                ...
                wh_int_iter_peek(it, reinterpret_cast<void *>(res_key), sizeof(res_key), &res_size, 
                                     reinterpret_cast<void *>(&store), sizeof(typename Steroids<O>::InfixStore), &dummy);
    ...
    static void PointQuery() {
        ...
        for (int32_t i = 1; i < 100; i++) {
            ...
            const uint32_t bits_to_zero_out = sizeof(uint64_t) * 8 - shared - ignore - Steroids<O>::base_implicit_size - s.infix_size_;
            ...
            SUBCASE("1") {
                ...
                if constexpr (O) {
                    ...
                    std::vector<uint64_t> left_store_infixes {0b0'01010101'01011,
                        0b0'11011011'00100, 0b0'11011011'00001,
                        0b1'01010101'01000, 0b1'01010101'01011,
                        0b1'11011011'00001};
                    ...
                    std::vector<uint64_t> right_store_infixes {0b1'01010101'10000,
                        0b1'01010101'01011, 0b1'01111111'11001,
                        0b1'11111100'01000, 0b1'11111100'00111};
            ...
            SUBCASE("2") {
                ...
                if constexpr (O) {
                    ...
                    std::vector<uint64_t> left_store_infixes {0b0'00000000'10100,
                        0b0'00000000'10101, 0b0'00000001'10111,
                        0b0'00000010'10000, 0b0'00000010'00001,
                        0b0'00000011'10111, 0b1'00000000'00001};
                    s.LoadListToInfixStore(*store, left_store_infixes.data(), left_store_infixes.size(), total_implicit);
                    ...
                    std::vector<uint64_t> right_store_infixes {0b0'10100000'11111,
                        0b0'11110101'01000, 0b0'11110101'00001,
                        0b1'01001011'01011};
                    s.LoadListToInfixStore(*store, right_store_infixes.data(), right_store_infixes.size(), total_implicit);
            ...
            SUBCASE("3") {
                ...
                if constexpr (O) {
                    ...
                    wh_int_iter_peek_ref(it, reinterpret_cast<const void **>(&key_ptr), &key_len,
                            reinterpret_cast<void **>(&store), &dummy);
                    {
                        const std::vector<uint32_t> occupieds_pos = {0, 160,
                            245, 331};
                        const std::vector<std::tuple<uint32_t, bool, uint64_t>> checks =
    ...
    static void BulkLoad() {
        ...
        SUBCASE("variable length") {
            ...
            Steroids<O> s(infix_size, string_keys.begin(), string_keys.end(), seed, load_factor);
...
private:
    template <bool O>
    static void AssertStoreContents(const Steroids<O>& s, const typename Steroids<O>::InfixStore& store,
                                    const std::vector<uint32_t>& occupieds_pos,
                                    const std::vector<std::tuple<uint32_t, bool, uint64_t>>& checks) {
...
TEST_SUITE("steroids") {
    TEST_CASE("insert") {
        SteroidsTests::Insert<false>();
    }

    TEST_CASE("random inserts") {
        SteroidsTests::RandomInsert<false>();
    }

    TEST_CASE("point query") {
        SteroidsTests::PointQuery<false>();
    }

    TEST_CASE("range query") {
        SteroidsTests::RangeQuery<false>();
    }

    TEST_CASE("delete") {
        SteroidsTests::Delete<false>();
    }

    TEST_CASE("shrink infix size") {
        SteroidsTests::ShrinkInfixSize<false>();
    }

    TEST_CASE("bulk load") {
        SteroidsTests::BulkLoad<false>();
    }
}

TEST_SUITE("steroids (int optimized)") {
    TEST_CASE("insert") {
        SteroidsTests::Insert<true>();
    }

    TEST_CASE("random inserts") {
        SteroidsTests::RandomInsert<true>();
    }

    TEST_CASE("point query") {
        SteroidsTests::PointQuery<true>();
    }

    TEST_CASE("range query") {
        SteroidsTests::RangeQuery<true>();
    }

    TEST_CASE("delete") {
        SteroidsTests::Delete<true>();
    }

    TEST_CASE("shrink infix size") {
        SteroidsTests::ShrinkInfixSize<true>();
    }

    TEST_CASE("bulk load") {
        SteroidsTests::BulkLoad<true>();
    }
}
