...
public:
    ...
    inner_node<T> *set_value(T &value) override {
        node_4_valued<T> *res = new node_4_valued<T>();
...
template <class T> class node_4_valued : public node_4<T> {
public:
...
template <class T>
node<T> **node_4<T>::find_child(uint8_t partial_key) {
...
template <class T> void node_4<T>::set_child(uint8_t partial_key, node<T> *child) {
...
template <class T> uint8_t node_4<T>::next_partial_key(uint8_t partial_key) const {
    for (int i = 0; i < n_children_; ++i) {
        if (keys_[i] >= partial_key) {
            return keys_[i];
        }
...
template <class T> uint8_t node_4<T>::prev_partial_key(uint8_t partial_key) const {
